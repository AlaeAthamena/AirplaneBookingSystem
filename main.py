...
class SeatBookingSystem:
    def __init__(self):
        self.num_rows = 7
        self.num_cols = 80
        # Initialize seats with default status 'F' (Free) and default type 'Economy'
        ...
        self.selected_seats = set()  # Store selected seat positions for multi-selection

        # Define seat types and their corresponding background colors
        self.seat_types = {
            'Economy': 'white',
            'First': 'white',  # Same color as Economy but shown differently via borders
            'Storage': 'lightgray',
            'Aisle': 'gray'
        }

    def mark_special_seats(self):
        """Sets up special seats like Aisles, Storage, and First class"""
        # Row D is the aisle (4th row, index 3)
        for col in range(self.num_cols):
            self.seats[3][col] = ('X', 'Aisle')

        # Mark storage seats in specific columns for all rows except aisle row
        ...
        # Mark columns 0 to 29 as First class seats if not already special
        for col in range(30):
            for row in range(self.num_rows):
    ...
    def get_seat_name(self, row, col):
        """Returns human-readable seat name like A1, B15, etc."""
        return f"{self.row_letters[row]}{col + 1}"

    def toggle_seat_selection(self, row, col):
        """Toggle seat selection/deselection for multiple bookings"""
        seat_pos = (row, col)
        status, seat_type = self.seats[row][col]

        # Prevent selecting aisle or storage seats
        if status not in ['F', 'R']:
            return False, f"Cannot select {seat_type} seat"
    ...
    def book_seats(self, priority_booking=False):
        """Book all currently selected seats"""
        ...
        for row, col in self.selected_seats:
            status, seat_type = self.seats[row][col]
            if status == 'F':
                self.seats[row][col] = ('R', seat_type)  # Mark as Reserved
    ...
    def free_seats(self):
        """Free all currently selected Reserved seats"""
        if not self.selected_seats:
            return False, "No seats selected"
        ...
        for row, col in self.selected_seats:
            status, seat_type = self.seats[row][col]
            if status == 'R':
                self.seats[row][col] = ('F', seat_type)  # Mark as Free again
    ...
    def get_seat_status(self, row, col):
        """Returns a status string for a given seat"""
...
class SeatBookingGUI:
    ...
    def create_widgets(self):
        """Create main interface widgets including buttons and labels"""
        ...
        ttk.Button(menu_frame, text="Book Seats", width=button_width, command=lambda: self.book_selected_seats(False)).grid(row=0, column=0, padx=padding, pady=padding)
        ttk.Button(menu_frame, text="Free Seats", width=button_width, command=self.free_selected_seats).grid(row=0, column=1, padx=padding, pady=padding)
        ttk.Button(menu_frame, text="Check Status", width=button_width, command=self.check_selected_status).grid(row=1, column=0, padx=padding, pady=padding)
        ttk.Button(menu_frame, text="Exit", width=button_width, command=self.root.quit).grid(row=1, column=1, padx=padding, pady=padding)
    ...
    def create_seating_display(self):
        """Creates a scrollable seating layout with clickable seats"""
        ...
        # Display column numbers above seat layout
        ...
        for row in range(self.booking_system.num_rows):
            ...
            for col in range(self.booking_system.num_cols):
                ...
                seat_button = tk.Label(self.seats_frame, text=status, width=2, relief="raised", borderwidth=1, fg='black')
    ...
    def update_seat_color(self, button, status, seat_type, is_selected=False):
        ...
        if is_selected:
            ...
            if status == 'R':
                ...
                if seat_type == 'First':
                    button.configure(bg=base_color, relief="solid", borderwidth=2, highlightbackground='gold', highlightthickness=2, fg='black')
                else:
                    button.configure(bg=base_color, relief="raised", fg='black')

    def on_seat_click(self, row, col):
        """Handles seat selection when user clicks a seat"""
    ...
    def update_seat_display(self):
        """Refresh seat buttons to reflect current state"""
        for row in range(self.booking_system.num_rows):
            for col in range(self.booking_system.num_cols):
                status, seat_type = self.booking_system.seats[row][col]
